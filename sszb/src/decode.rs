...
pub enum DecodeError {
    ...
    /// Some item has a `sszb_fixed_len` of zero. This is illegal.
    ZeroLengthItem,
...
impl<'a> SszDecoderBuilder<'a> {
    ...
    pub fn register_anonymous_variable_length_item(&mut self) -> Result<(), DecodeError> {
        ...
        impl Decode for Anonymous {
            ...
            fn from_sszb_bytes(_bytes: &[u8]) -> Result<Self, DecodeError> {
...
/// use sszb::{Decode, Encode, SszDecoder, SszDecoderBuilder};
///
...
/// }
///
/// fn sszb_decoding_example() {
///     let foo = Foo {
///         a: 42,
///         b: vec![1, 3, 3, 7]
///     };
///
///     let bytes = foo.as_sszb_bytes();
///
...
impl<'a> SszDecoder<'a> {
    ...
    pub fn decode_next<T: Decode>(&mut self) -> Result<T, DecodeError> {
        self.decode_next_with(|slice| T::from_sszb_bytes(slice))
    }
...
/// Decode bytes as a big-endian usize, returning an `Err` if `bytes.len() !=
/// BYTES_PER_LENGTH_OFFSET`.
